...
FORM_MAIN, _ = loadUiType('ui/khata_details.ui')


class KhataWindow(QMainWindow, FORM_MAIN):
    def __init__(self):
    ...
    def handle_buttons(self):
        self.btn_save.clicked.connect(self.add_khata)
        self.btn_clear.clicked.connect(self.clear_fields)
        self.btn_cancel.clicked.connect(self.close)

    def add_khata(self):
        name=self.txt_khata.text()
        if name:
            try:
                self.db.insert(table_name="khata",columns="khata_name",values=f"'{name}'")
                self.db.conn.commit()
                QMessageBox.information(self, "Success", "Khata added successfully")
                self.close()
            except Exception as e:
                QMessageBox.warning(self, "Error", f"Khata not added {e}")

        else:
            QMessageBox.warning(self, "Error", "Khata name is required")

    def clear_fields(self):
        self.txt_khata.setText("")
